...
class TestReservation(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_customers = [{"id": "C1", "name": "Klag", "email": "k@mail.com"}]
        self.mock_hotels = [{"id": "H1", "name": "Tec Hotel", "location": "Toluca", "rooms": 10}]
    ...
    def test_create_reservation_success(self):
        ...
        result = self.res_sys.create_reservation("R1", "C1", "H1", self.mock_customers, self.mock_hotels)
        self.assertTrue(os.path.exists(self.test_file))

    def test_create_reservation_invalid_customer(self):
        """Caso Negativo 1: Cliente no existe."""
        self.res_sys.create_reservation("R2", "999", "H1", self.mock_customers, self.mock_hotels)
        res = self.res_sys.display_reservations()
        self.assertEqual(len(res), 0)

    def test_create_reservation_invalid_hotel(self):
        """Caso Negativo 2: Hotel no existe."""
        self.res_sys.create_reservation("R3", "C1", "999", self.mock_customers, self.mock_hotels)
        res = self.res_sys.display_reservations()
        self.assertEqual(len(res), 0)
